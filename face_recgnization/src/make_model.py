...
# tensorflow是目前业界最流行的深度学习框架，在图像，语音，文本，目标检测等领域都有深入的应用。是该实验的核心，主要用于定义占位符，定义变量，创建卷积神经网络模型
...
def make_model():
    ...
    # 利用numpy中的arange函数生成一个跟样本数据集一样大小的等差数组
    ...
    # 定义卷积神经网络模型model，三个变量分别是输入张量input_tensor，用于区分训练过程还是验证过程的train，正则项regularizer。
    def model(input_tensor, train, regularizer=None):
        # 定义一个作用域：layer1-conv1，在该作用域下面可以定义相同名称的变量（用于变量）
        with tf.variable_scope('layer1-conv1'):
            ...
            # 定义激活函数：利用bias_add给每个节点都加上偏置项，然后利用relu函数去线性化
        ...
        with tf.variable_scope('layer11-fc3'):
            ...
            fc3_biases = tf.get_variable(
                "bias", [5], initializer=tf.constant_initializer(0.1))
    ...
    # tf.argmax返回最大数值的下标，tf.cast转换数据格式为int32，tf.equal对比两个序列中元素值，相等返回true不等返回false
    correct_prediction = tf.equal(tf.cast(tf.argmax(logits, 1), tf.int32), y_)
    # 计算模型准确性：tf.cast转换数据格式为float32，tf.reduce_mean函数用于求平均值
    acc = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))

    # 7.train and test
    # 定义一个函数，按批次取数据
    # 定义批量提取数据函数：inputs为输入数据，targets为标签数据，batch_size为每一个batch数据集的大小，shuffle代表是否打乱
    def minibatches(inputs=None, targets=None, batch_size=None, shuffle=False):
        ...
        # 生成以batch_size为等差的等差数列
        for start_idx in range(0, len(inputs) - batch_size + 1, batch_size):
            if shuffle:
                # 在该等差数列中，如果需要打乱数据集，则提取相应的样本数据索引
                excerpt = indices[start_idx:start_idx + batch_size]
            else:
                # 在该等差数列中，如果不需要打乱数据集，则提取相应的样本数据索引
    ...
    for epoch in range(n_epoch):                    # 总共有5个epoch，开始遍历每一层的epoch
        # 在每层epoch的开始训练之前，打印当前epoch层级
        print("epoch:", epoch+1)
        # 返回每个epoch开始训练时候的时间，用于计算每个epoch以及总共消耗了多少时间
        ...
        for x_train_a, y_train_a in minibatches(x_train, y_train, batch_size, shuffle=True):
            ...
            train_loss += err
            train_acc += ac
            n_batch += 1                                    # 将每次迭代的损失值err，准确率ac，批次数n_batch进行累计
        ...
        # 利用minibatches函数从验证数据中以不打乱顺序的方式提取批数据进行验证，批大小为64
    ...
    # 利用saver的save函数对sess运算后的模型进行保存，保存路径为model_path
